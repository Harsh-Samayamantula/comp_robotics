...
def build_rrt_star(robot_type, start_config, goal_config, environment, goal_radius=0.1, max_nodes=5000, step_size=0.4, radius=1.0, animation_func=None):
    tree = nx.Graph()
...
def main(robot_type, start_config, goal_config, map_file, goal_radius):
    ...
    rrt_growth_animation_func = lambda tree, env, start, goal: animate_rrt(tree, env, start, goal, filename='component_5_4.gif')

    # Build RRT
    tree, goal_node = build_rrt_star(robot_type, start_config, goal_config, environment, goal_radius=goal_radius, animation_func=rrt_growth_animation_func)
